...
still drained. Once those pools are closed, the ``FuturesSession`` cannot be
used for more requests. A supplied session is not closed automatically; when
both an executor and session are supplied, their lifecycle remains the
